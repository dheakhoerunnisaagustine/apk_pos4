    ...
    <div style="max-width:1200px; margin:auto;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;">
            ...
            @can('create', App\Models\Produk::class)
                <a href="{{ route('produk.create') }}" style="background:#D97736; color:white; padding:12px 25px; border-radius:14px; text-decoration:none; font-weight:600; box-shadow:0 8px 18px rgba(217,119,54,.25);">
                    ➕ Tambah Produk
                </a>
            @endcan
        </div>
            ...
            <div style="padding:25px;">
                <form action="{{ route('produk.index') }}" method="GET">
                    <div style="display:flex; gap:10px;">
                        ...
                        >
                        <button type="submit" style="background:#8B5E3C; color:white; border:none; padding:13px 22px; border-radius:14px; font-weight:600; cursor:pointer;">
                            🔍 Cari
                        </button>
                    </div>
                </form>
            ...
            <div style="overflow-x:auto; padding:0 25px 25px;">
                <table width="100%" style="border-collapse:collapse;">
                    ...
                    <tbody>
                        @forelse($products as $product)
                            <tr style="border-bottom:1px solid #F0E2D3;">
                                ...
                                <td style="text-align:center; padding:15px;">
                                    @if($product->foto)
                                        <img src="{{ asset('storage/' . $product->foto) }}" style="width:52px; height:52px; object-fit:cover; border-radius:14px; box-shadow:0 2px 6px rgba(0,0,0,0.08);">
                                    @else
                                ...
                                <td style="text-align:center; padding:15px;">
                                    <div style="display:inline-flex; gap:6px; align-items:center;">
                                        @can('view', $product)
                                            <a href="{{ route('produk.show', $product->id) }}" title="Detail" style="background:#DDEBE7; color:#35685A; padding:8px 12px; border-radius:10px; text-decoration:none; font-weight:600; font-size:13px; display:inline-flex; align-items:center;">
                                                👁
                                            </a>
                                        @endcan

                                        @can('update', $product)
                                            <a href="{{ route('produk.edit', $product) }}" title="Edit" style="background:#F5D7B8; color:#6B4328; padding:8px 12px; border-radius:10px; text-decoration:none; font-weight:600; font-size:13px; display:inline-flex; align-items:center;">
                                                ✏️
                                            </a>
                                        @endcan
                                        ...
                                        @can('delete', $product)
                                            <form action="{{ route('produk.destroy', $product) }}" method="POST" style="display:inline; margin:0;">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirm('Yakin hapus produk ini?')" title="Hapus" style="background:#E8B4A8; color:#7A2E22; border:none; padding:8px 12px; border-radius:10px; cursor:pointer; font-weight:600; font-size:13px; display:inline-flex; align-items:center;">
                                                    🗑
                                                </button>
                                            </form>
                                        @endcan
                        ...
                        @empty
                            <tr>
                                <td colspan="7" style="padding:30px; text-align:center; color:#8C7A6B;">
                                    Belum ada produk 🥐
                                </td>
                            </tr>
                        @endforelse
